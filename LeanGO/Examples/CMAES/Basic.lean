...
The original CMA-ES is defined in `LeanGO.Examples.CMAES.Original`.
-/

abbrev ℝ_ (d n : ℕ) := Fin n → EuclideanSpace ℝ (Fin d)

variable (d lam : ℕ) {β : Type*} [MeasurableSpace β]
  {mean : (n : ℕ) → prod_iter_image (ℝ_ d lam) β n → EuclideanSpace ℝ (Fin d)}
  (hmean : ∀ n, Measurable <| mean n)
  {covar : (n : ℕ) → prod_iter_image (ℝ_ d lam) β n → Matrix (Fin d) (Fin d) ℝ}
  (hcovar : ∀ n, Measurable <| covar n)

noncomputable def CMAKernel (n : ℕ) :
    Kernel (prod_iter_image (ℝ_ d lam) β n) (ℝ_ d lam) where
