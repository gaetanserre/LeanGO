...
We define the {name Decision}`Decision` structure as a special case of the `Algorithm` structure, for which the Markov kernel at iteration $`n` is defined through the decision rules $`(D_n)_{n \in \mathbb{N}}`. Measurability assumptions are required on the decision rules for the kernels to be well-defined and we only support $`\{0, 1\}`-valued decision rules at the moment.
